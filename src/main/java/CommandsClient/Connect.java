...
public class Connect implements CommandClient {
    ...
    public void execute(String argument) {
        ...
        try {
            ...
            try {
                ...
                out.println("shareInfo");
                // listen the response from the server
                out.println(storage.getClientAddress()+ " " + storage.getClientListeningPort());
                String response = "";
                while (!(response = in.readLine()).equalsIgnoreCase("received")) {
    ...
    @Override
    public String help() {
        return "Connect to a server with " + Utils.ANSI_BLUE + storage.getServerAddress() + Utils.ANSI_RESET + ":"
                + Utils.ANSI_BLUE + storage.getServerPort() + Utils.ANSI_RESET
                + " as default (you can change it with the command " + Utils.ANSI_YELLOW + "init" + Utils.ANSI_RESET
                + ")";
    }
}
